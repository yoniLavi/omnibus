...
# download/install Ruby 1.9.2
$client.DownloadFile("http://rubyforge.org/frs/download.php/75127/rubyinstaller-1.9.2-p290.exe","$ENV:TEMP\rubyinstaller-1.9.2.exe")
Start-Process -FilePath "$ENV:TEMP\rubyinstaller-1.9.2.exe" '/verysilent /tasks="assocfiles,modpath"' -Wait

# add Ruby and Git to the PATH and persist it
$ENV:PATH = "$ENV:PATH;C:\Ruby192\bin;C:\git\cmd"
...
Start-Process -FilePath "gem" 'install rdp-ruby-wmi windows-api windows-pr win32-process --no-rdoc --no-ri' -Wait
Start-Process -FilePath "gem" 'install chef -v 0.10.8 --no-rdoc --no-ri --verbose' -Wait
