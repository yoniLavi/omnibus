...
New-Item C:\chef -type directory > $ENV:TEMP\omnibus.out 2>&1
$json_attribs = @"
{
  "aws": {
...
$json_attribs | Out-File -Encoding ASCII C:\chef\omnibus.json
...
$script = {
  ...
  Start-Process -FilePath 'chef-solo' '-c c:/chef/solo.rb -j c:/chef/omnibus.json' -Wait -NoNewWindow
}
